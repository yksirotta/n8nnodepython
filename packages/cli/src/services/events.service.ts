...
export class EventsService extends EventEmitter {
	constructor(private userService: UserService, private repository: WorkflowStatisticsRepository) {
	...
	async workflowExecutionCompleted(workflowData: IWorkflowBase, runData: IRun): Promise<void> {
		...
		try {
			...
			if (name === 'production_success' && upsertResult === 'insert') {
				const owner = await this.userService.getWorkflowOwner(workflowId);
				const metrics = {
					user_id: owner.id,
				...
				if (!owner.settings?.userActivated) {
					await this.userService.updateUserSettings(owner.id, {
