...
import { sendErrorResponse, sendSuccessResponse } from '@/ResponseHelper';
...
export abstract class AbstractWebhooks {
	...
	abstract handleWebhookRequest(req: WebhookRequest, res: Response): Promise<IResponseCallbackData>;

	async handleRequest(req: WebhookRequest | WebhookCORSRequest, res: Response) {
		...
		if ('origin' in req.headers) {
			...
			const requestedMethod =
				method === 'OPTIONS'
					...
					: method;
			if (this.findAccessControlOptions && requestedMethod) {
				const options = await this.findAccessControlOptions(path, requestedMethod);
		...
		let response;
		try {
			response = await this.handleWebhookRequest(req, res);
		} catch (error) {
			return sendErrorResponse(res, error as Error);
		}
