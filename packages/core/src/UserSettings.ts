...
export async function prepareUserSettings(): Promise<IUserSettings> {
	...
	if (userSettings !== undefined) {
		if (userSettings.instanceId === undefined) {
			userSettings.instanceId = generateInstanceId(userSettings.encryptionKey);
	...
	const encryptionKey =
		process.env[ENCRYPTION_KEY_ENV_OVERWRITE] ?? randomBytes(24).toString('base64');
	userSettings = {
		encryptionKey,
		instanceId: generateInstanceId(encryptionKey),
	};
...
function generateInstanceId(key?: string) {
	const hash = key
		? createHash('sha256')
